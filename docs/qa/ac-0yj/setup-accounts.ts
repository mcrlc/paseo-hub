/* oxlint-disable */
// Usage: npx tsx docs/qa/ac-0yj/setup-accounts.ts <origin> <state-dir>
...
const [origin = "http://127.0.0.1:4107", stateDir = "/tmp/ac0yj-state"] = process.argv.slice(2);
const OWNER = { email: "owner@ac0yj.test", password: "ac0yj-owner-password" };
const OWNER_NEW_PASSWORD = "ac0yj-owner-password-2";
...
async function main() {
  ...
  try {
    ...
    for (const role of ["admin", "member"] as const) {
      const link = await invite(ownerPage, owner, slug, `${role}@ac0yj.test`, role);
...
async function join(browser: Browser, link: string, role: string, statePath: string) {
  ...
  await form.getByLabel("Password").fill(`ac0yj-${role}-password`);

...
curl -s -w '\nhttp=%{http_code}\n' "https://api.sprites.dev/v1/gateway/github/sE28wY8RezD6zhaB19eN-Q/user" | head -c 400; echo
curl -s -o /dev/null -w 'repo http=%{http_code}\n' "https://api.sprites.dev/v1/gateway/github/sE28wY8RezD6zhaB19eN-Q/repos/mcrlc/paseo-hub"
echo "=== git ls-remote through the gateway ==="
GIT_TERMINAL_PROMPT=0 git ls-remote "https://api.sprites.dev/v1/gateway/github/sE28wY8RezD6zhaB19eN-Q/mcrlc/paseo-hub.git" 2>&1 | head -5
...
curl -s -o /dev/null -w 'info/refs http=%{http_code} type=%{content_type}\n' "https://api.sprites.dev/v1/gateway/github/sE28wY8RezD6zhaB19eN-Q/mcrlc/paseo-hub.git/info/refs?service=git-upload-pack"
curl -s -o /dev/null -w 'repos info/refs http=%{http_code}\n' "https://api.sprites.dev/v1/gateway/github/sE28wY8RezD6zhaB19eN-Q/repos/mcrlc/paseo-hub.git/info/refs?service=git-upload-pack"
echo "=== tarball via REST through the gateway (fallback for clone) ==="
curl -s -o /tmp/tb.tgz -L -w 'tarball http=%{http_code} size=%{size_download}\n' "https://api.sprites.dev/v1/gateway/github/sE28wY8RezD6zhaB19eN-Q/repos/mcrlc/paseo-hub/tarball/main"; tar tzf /tmp/tb.tgz 2>/dev/null | head -3

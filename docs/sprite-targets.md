...
The authored `bootstrap` clones, using a deploy key or token the author puts in `target.env` or writes to the sprite during bootstrap. The docs carry the two measured gotchas: run the Claude Code postinstall explicitly, and use absolute paths because the exec shell is non-login. A Hub-driven clone with a machine-scoped lease is a follow-up.
...
| Execution terminal                   | `release`; provider pauses about 30 s later                                                 |
...
- **First boot is about a minute.** Bootstrap installs in about 25 s and the daemon's first start pulls about 1 GB of speech models. Warm and cold wakes are about a second.

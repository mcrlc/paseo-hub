...
- **First boot is about a minute.** Bootstrap installs in about 25 s and the daemon's first start pulls about 1 GB of speech models. A warm wake is about a second and a cold wake normally a few seconds (2 s in the beta). The one slow wake measured, from cold in the spike, took 64 s to a daemon reconnect with a 30 s `git rev-parse` timeout inside the daemon; its cause was not recorded, and an arrival that meets such a wake defers about a minute.

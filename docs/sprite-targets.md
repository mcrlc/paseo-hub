...
- The Sprites org token is stored in plain jsonb like other operator configuration. It can exec into every sprite in the org, so operators self-hosting Hub should treat the database as holding shell access to their sprites.

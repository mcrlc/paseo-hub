...
- **Sprite credentials outlive executions.** The organization's daemon environment, set on Settings → Sprites and including the Claude subscription token, is written into every sprite's daemon service. It and any long-lived GitHub credential placed on the sprite are not per-execution leases: they live as long as the sprite and reach every agent on it, and untrusted pull request comments reach an agent that holds them. Use a fine-grained GitHub token scoped to the one repository. Nothing secret belongs in trigger YAML. A trigger that uses `run.github` instead gets leased tokens revoked at terminal and gives up continuation across idle days.

...
- **The database holds shell access to your sprites.** Hub stores the Sprites org token, and the organization's daemon environment, as plain text in the `organization_sprites_configuration` table. There is no secret encryption layer. The org token can execute commands in every sprite in the organization, read and write their filesystems, and destroy them. Protect the database, its backups, and read access to them accordingly.
- **Sprite credentials outlive executions.** The organization's daemon environment, set on Settings → Sprites and including the Claude subscription token, is written into every sprite's daemon service and into the environment of its `bootstrap` run. It and any long-lived GitHub credential placed on the sprite are not per-execution leases: they live as long as the sprite and reach every agent on it, and untrusted pull request comments reach an agent that holds them. Use a fine-grained GitHub token scoped to the one repository. Nothing secret belongs in trigger YAML: the target's own `env` is not treated as secret and is not redacted from logs, while values of eight characters or more from the daemon environment are. A trigger that uses `run.github` instead gets leased tokens revoked at terminal and gives up continuation across idle days.
